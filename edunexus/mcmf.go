...
func (a *Backend) RunGameFlowNetwork(nodes []MCMFNode, edges []MCMFEdge) {
	...
	go func() {
		...
		// Map UI Node IDs to 0-indexed internal indices (assuming IDs are sequential or simple for this prototype)
		// For robustness, we should ideally use a map, but we'll assume node IDs match array indices if they are 0 to N-1
		// Or we just find source and sink. Let's assume nodes list dictates indices.
		sourceIdx := -1
		sinkIdx := -1
		for i, n := range nodes {
			if n.Type == "source" {
